...
export function ProductFilters({ categories }: ProductFiltersProps) {
  ...
  return (
    <div className='space-y-6 rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-950'>
      ...
      <div>
        ...
        <Slider
          ...
          onValueCommit={([min, max]) =>
            updateParams({ minPrice: min > 0 ? min : null, maxPrice: max < 5000 ? max : null })
          }
          className={`mt-4 ${
            isPriceActive ? '**:[[role=slider]]:border-amber-500 **:[[role=slider]]:ring-amber-500' : ''
          }`}
        />
      </div>
        ...
        <label className='flex cursor-pointer items-center gap-3'>
          ...
          <span
            className={`text-sm font-medium ${
              isInStockActive ? 'text-zinc-900 dark:text-zinc-100' : 'text-zinc-700 dark:text-zinc-300'
            }`}>
            Show only in-stock
            {isInStockActive && (
      ...
      {/* Sort */}
      <div>
        <span className='mb-2 block text-sm font-medium text-zinc-700 dark:text-zinc-300'>Sort By</span>
  ...
  return (
    <div className='mb-2 flex items-center justify-between'>
      <span
        ...
        {children}
        {isActive && (
          <Badge className='ml-2 h-5 bg-amber-500 px-1.5 text-xs text-white hover:bg-amber-500'>Active</Badge>
        )}
      </span>
