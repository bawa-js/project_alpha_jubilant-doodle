...
interface EmptyStateProps {
  ...
  action?: { label: string; onClick?: () => void; href?: string; disabled?: boolean; icon?: LucideIcon }
...
export function EmptyState({
  ...
  return (
    <div className={cn('flex flex-col items-center justify-center text-center', config.container, className)}>
      ...
      {description && (
        <p className={cn('max-w-sm text-zinc-500 dark:text-zinc-400', config.description)}>{description}</p>
      )}
      {ActionButton}
    </div>
  )
