...
export function AddToCartButton({ productId, name, price, image, stock, className }: AddToCartButtonProps) {
  ...
  return (
    <div
      ...
      <Button variant='ghost' size='icon' className='h-full flex-1 rounded-r-none' onClick={handleDecrement}>
        <Minus className='h-4 w-4' />
      </Button>
      <span className='flex-1 text-center text-sm font-semibold tabular-nums'>{quantityInCart}</span>
      <Button
        variant='ghost'
        size='icon'
