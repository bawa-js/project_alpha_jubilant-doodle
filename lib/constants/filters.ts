...
export const COLORS_SANITY_LIST = COLORS.map(({ value, label }) => ({ title: label, value }))

/** Materials formatted for Sanity schema options.list */
export const MATERIALS_SANITY_LIST = MATERIALS.map(({ value, label }) => ({ title: label, value }))
